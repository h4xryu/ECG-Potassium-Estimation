...
def encode_k_class(k_value):
    ...
    Args:
        k_value (float): Serum potassium concentration (mEq/L)

    Returns:
        int: Encoded class (0 to 5)
    """
    if k_value <= 1.5:
        return 0
    elif 1.5 < k_value <= 2.5:
        return 1
        ...
        return 4
    elif 6.5 < k_value <= 7.5:
        return 5
    else:
        return 6


def decode_k_concentrations(predictions):
    """
    Decode a list of 60-point sigmoid outputs to potassium concentration values.

    Args:
        predictions_list (list of list): List of 60-point sigmoid outputs

    Returns:
        list: Decoded potassium concentrations
    """
    if isinstance(predictions, (float, np.float32)):
        predictions = [predictions]  # Convert to list if single value
        # 각 시그모이드 벡터의 합을 구하고 디코딩
    k_concentration = round((math.floor(sum(predictions)) / 10 + 3.9),1)

    if k_concentration < 4.0:
        k_concentration = 4.0
...
def encode_k_concentration(k_value, bins=40, min_k=4.0, max_k=8.0):
    """
    Encodes the K+ concentration into a binary vector of a specified length.

    Args:
        k_value (float): Serum potassium concentration (mEq/L)
        bins (int): Number of bins for encoding
        min_k (float): Minimum K+ concentration (mEq/L)
        max_k (float): Maximum K+ concentration (mEq/L)

    Returns:
        np.ndarray: Binary vector of length `bins`
    """
    if k_value <= min_k:
        # If below minimum, return all zeros
        return np.zeros(bins, dtype=int)
    elif k_value >= max_k:
        # If above maximum, return all ones
        return np.ones(bins, dtype=int)

    # Calculate bin step size
    step = (max_k - min_k) / bins
    cutoff = int((k_value -min_k)/ step)  # Determine the cutoff index

    # Create binary vector
    encoded = np.zeros(bins, dtype=int)
    encoded[:cutoff + 1] = 1  # Fill with 1s up to the cutoff index
    return encoded
